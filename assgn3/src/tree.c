...
char *nodeNames[42] = {"program", "declList", "decl", "varDecl", "typeSpecifier",
                       ...
                       "arrayDecl", "char", "string", "funcTypeName", "logicalop", "unaryop", "if", "else", "while", "return", "="};
...
void printAst(tree *node, int nestLevel) {
      ...
      if(strcmp(nodeName,"identifier") == 0){
          ...
          printf("%s,%s\n", nodeName,typeNames[node->val]);
           ...
           || strcmp(nodeName,"unaryop") == 0 || strcmp(nodeName,"logicalop") == 0){
...
//-------Error checking for duplicate entry ----//
void duplicate(tree *node){
