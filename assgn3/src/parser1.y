...
var             : ID
				{
					...
					if(idEntry != NULL){
						...
						yyerror("Use of undeclared symbol");
					}
			//already declared
					struct strEntry *currEntry = (struct strEntry *) malloc(sizeof(struct strEntry));
					tree *newNode = NULL;
					if ((currEntry->id ) == id->name)
						yyerror("variable previously defined");
					else
						{
						newNode = maketreeWithStrVal(IDENTIFIER, currEntry->id);
						newNode->scope = currEntry->scope;
						newNode->data_type = currEntry->type->data_type;
						newNode->sym_type = currEntry->type->symbol_type;
						}
